...
{  
    ...
    {     
		...
		if(cmemb==nmemb)							//找到了连续nmemb个空内存块
		{
            ...
            return (offset*memblksize[memx]);//返回偏移地址   把最后的这个空内存块的地址返回指针p，完成一次分配  
		}                                    //offset*memblksize[memx]表示内存块个数乘以一个内存块的大小 从而表示偏移地址
    }  
...
{  
    ...
    {  
        ...
        int nmemb=mallco_dev.memmap[memx][index];	//内存块数量   内存管理状态表中存放的是连续内存块数量
        for(i=0;i<nmemb;i++)  						//内存块清零
        {  
            mallco_dev.memmap[memx][index+i]=0;    //管理状态表清零 代表释放内存 可以进行再次使用
        }  
        return 0;  
    }else return 2;//偏移超区了.  
}  
